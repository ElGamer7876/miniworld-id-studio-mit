...
import { EVENTS, METHODS, eventById, methodByKey } from './catalog';
...
function renderAction(action: StudioAction, index: number): string {
  const details = action.type === 'api' ? `<select data-action-method="${index}">${METHODS.map((method) => `<option value="${method.key}" ${method.key === action.method ? 'selected' : ''}>${html(method.name)} · ${method.key}</option>`).join('')}</select><input data-action-targets="${index}" value="${html(action.targets || '')}" placeholder="Resultados opcionales: code, objid"><textarea data-action-value="${index}" placeholder="Argumentos separados por coma">${html(action.value)}</textarea>` : action.type === 'if' ? `<input data-action-condition="${index}" value="${html(action.condition || action.value)}" placeholder="e.eventobjid ~= nil">` : `<textarea data-action-value="${index}">${html(action.value)}</textarea>`;
...
function bindActionEditors(host: HTMLElement, trigger: StudioTrigger): void {
  ...
  change('[data-action-label]', (action, input) => action.label = input.value); change('[data-action-value]', (action, input) => action.value = input.value); change('[data-action-targets]', (action, input) => action.targets = input.value); change('[data-action-method]', (action, input) => { action.method = input.value; if (project.settings.easyMode) action.label = methodByKey(input.value)?.name || input.value; }); change('[data-action-condition]', (action, input) => { action.condition = input.value; action.value = input.value; });
...
function addAction(type: ActionType, method = ''): void { const before = snap(), action = createAction(type); if (type === 'api' && method) { action.method = method; action.label = project.settings.easyMode ? methodByKey(method)?.name || method : method; } activeTrigger().actions.push(action); commit(before, 'Acción añadida'); }
